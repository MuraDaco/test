...
extern LboxIstanzaDynStruct uiLboxIstanzaDyn;
...
extern _ROM unsigned char wdButtonsTaskFlags[9][2];
...
extern _ROM EventTableDef wdButtonsTaskFunctions[];
...
extern _ROM EventTableDef uiLboxEvent[];
...
_EOS_REENTRANT void uiLboxVisualizza		(char element_id, BaseCoordinateStruct coordinate)
{
   ...
   for(i=0;i<uiLboxIstanzaDyn.PtrIstanzaElemento->row_number;i++)
   {
      // visualizza elemento lista
      uiLboxIstanzaDyn.PtrLboxBaseElemento = BaseElemento + uiLboxIstanzaDyn.PtrLboxListaElemento->tipo_elemento;
      uiLboxIstanzaDyn.PtrLboxBaseElemento->InitLight(uiLboxIstanzaDyn.PtrLboxListaElemento->id_elemento);
